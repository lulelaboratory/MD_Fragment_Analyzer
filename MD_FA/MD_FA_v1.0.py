...
# --------------------------
@jit(nopython=True)
def compute_fragments_cpu(positions, radii, scale, n):
    bonds = []
    for i in range(n):
...
# --------------------------
def compute_fragments(atoms, scale=1.2, use_gpu=False):
    ...
    if use_gpu:
        ...
        bonds = compute_fragments_cpu(positions, radii, scale, n)
...
def process_frame(args):
    i, atoms, use_gpu, gpu_id = args
    if use_gpu and gpu_id is not None:
        cuda.select_device(gpu_id)
    frags = compute_fragments(atoms, use_gpu=use_gpu)
    frag_counts = Counter(frags)
    return i, list(frag_counts.items())

# --------------------------
# GUI Class using Tkinter (Revised Layout with GPU Selection & Close Button)
# --------------------------
    ...
    def run_counting(self):
        ...
        if self.use_gpu.get():
            ...
            if selected == "All GPUs":
                num_gpus = len(cuda.gpus)
                args = [(i, atoms, True, i % num_gpus) for i, atoms in enumerate(frames)]
            else:
                gpu_id = int(selected.split()[1])
                args = [(i, atoms, True, gpu_id) for i, atoms in enumerate(frames)]
        else:
            args = [(i, atoms, False, None) for i, atoms in enumerate(frames)]
